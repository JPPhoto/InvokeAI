...
<CardGrid>
  ...
  <Card title="Extend Video - LTX-2" icon="clock">
    Continue an existing clip. The continuation starts from the source's last few frames **and its closing sound**, and joins onto it with a cross-fade over the same span, so the picture and the soundtrack both carry across the join. `context_frames` sets how much of the source is carried in.
  </Card>
  <Card title="Extend Video to Image - LTX-2" icon="clock">
    The same, with a **destination image**: the continuation ends on your image. Give it the source clip's aspect ratio.
  </Card>
...
* **Frames** must be on LTX-2's 8·n + 1 grid (9, 17, … 121, … 241); unlike the Video panel, the workflows do not snap other values, and the run fails at the Denoise node. The frame rate sets how much audio is generated, as well as the clip's duration.
* **Extending:** `context_frames` defaults to 17 (about 0.7 s at 24 fps) — enough for motion, but only about one syllable of sound. For singing or other rhythmic material, raise it along the same grid (25, 33, 41, 49 …). `num_frames` *includes* the context, so the extension adds `num_frames − context_frames` new frames; raise both together.
...
The **Notes** panel of each lists the models to select: an LTX-2.5 transformer, the LTX-2.5 Components install, and the Gemma-4 text encoder.
:::
