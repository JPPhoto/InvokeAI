...
A first-generation PiD decoder download is roughly 5 GB, a PiD 1.5 decoder download roughly 2.8 GB, and the shared Gemma-2 encoder is roughly 5 GB. Once loaded, both generations take about the same memory: the decoder runs in full precision.
...
Only the `_4step` builds work: the files without it are NVIDIA's undistilled teacher decoders, which need a different sampler, and are rejected during model identification — as are the `int8_convrot` builds, which are not supported yet.
</Aside>
