...
boundary does not carry data. The data still moves through the ports and edges you connect.
...
`ForReturn.output` is optional. Leave it disconnected when the loop is being used only for its effects or when the
result is carried through loop state instead.
...
The first four outputs belong inside the loop body. The last two become available after the loop finishes.

## Returning Results

`ForReturn` closes the loop body. It has three useful inputs:
...
There is no need to connect `ForReturn` back to `For` manually. The loop connection is represented by the matching
`For` and `ForReturn` nodes and their body boundary in the editor.
