...
One exact serial two-`Iterate` body is also supported: `For.item` feeds a preparation node and the first `Iterate`;
its item feeds a second preparation node and `Iterate`, followed by one work node, one `Collect`, and `ForReturn`.
For this bounded pattern, use a non-empty literal outer collection and one consumer of `For.output_collection`.
Other arrangements, such as sibling internal `Iterate` nodes or an independent external `Iterate` feeding a `For` body,
are unsupported. The backend has a bounded
