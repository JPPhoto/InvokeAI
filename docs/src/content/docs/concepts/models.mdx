...
Comfy-Org's [`Comfy-Org/Ideogram-4`](https://huggingface.co/Comfy-Org/Ideogram-4) repackage is ungated (the same non-commercial license still applies) and ships the parts separately. Ideogram 4 runs **two** transformers — a conditional branch and an unconditional one, both active at every step — so a single-file install is four models, and the **Ideogram 4 (single file)** starter installs all four together:

- `diffusion_models/ideogram4_fp8_scaled.safetensors` — the conditional branch, selected as the model
- `diffusion_models/ideogram4_unconditional_fp8_scaled.safetensors` — the unconditional branch
...
Pick the unconditional branch, the encoder and the VAE in the **Components** section next to the model. The `int8_convrot` and `nvfp4` repackages in that repo are **not** supported yet and are refused at install.

:::note[Memory]
Both branches stay resident, so the fp8 pair needs roughly twice one file's size. Without `fp8_compute` the scaled fp8 weights are expanded to bf16 on load — about 35 GB for the pair — so on 24 GB cards the bundled nf4 build remains the smaller option.
:::
