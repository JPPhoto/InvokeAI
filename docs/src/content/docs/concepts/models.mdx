...
| **Ideogram 4 (single file, fp8)** — `fp8_scaled` | 8.6 GB | 8.7 GB with `fp8_compute`, ~17 GB without |
...
**Everywhere else, use int8.** Without `fp8_compute` the fp8 weights are expanded to bf16 on load — about 35 GB for the pair, which does not fit a 24 GB card — while int8 keeps its stored codes and dequantizes them per forward, on any device and with no setting to turn on. Those ~10 s are what the dequantization costs, and they buy the difference between the model fitting and not fitting.
