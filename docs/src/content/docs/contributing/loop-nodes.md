...
(one literal outer collection, or the exact statically non-empty `CollectionConcat` producer for the three-level case, one direct child at each level, no continuation nodes) and the exact
