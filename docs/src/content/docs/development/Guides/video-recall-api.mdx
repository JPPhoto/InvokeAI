...
3. **Event** — the backend emits one `video_recall_requested` socket event to the
   ...
   it.
4. **Frontend** — the user's open frontends apply the event to the Video panel.

:::note
The webv2 handler for `video_recall_requested` ships separately from these
endpoints. Until it is present, the endpoints validate, upload and emit, but no
client acts on the event.
:::
...
| `strict` | `false` | Forwarded to the frontend. `false` asks it to change only the fields you send; `true` asks it to treat the body as a whole generation record, clearing LoRAs and media you do not send. |
...
Whether the video is usable depends on the model selected in the panel: an
initial video needs a model that can extend a clip, and a reference video needs
a model that takes reference videos.
