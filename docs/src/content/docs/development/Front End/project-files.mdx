...
document, revision, project name, board name, and floor as one transaction. This is the activation
seam for a future canvas v3 document: the first v3 edit must send the v3 document and
`minimum_canvas_schema_version: 3` in the same optimistic-concurrency save. An older client then
defaults to a declared maximum of v2 and is refused before it can overwrite that document.
...
Requests that omit negotiation fields default to v2. That preserves compatibility during protocol
rollout while remaining safe for future documents: an undeclared older client is treated as v2,
not as unrestricted. The frontend's maximum lives in `canvasSchemaVersion.ts`; the canvas loader's
