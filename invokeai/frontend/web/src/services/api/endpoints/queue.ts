  ...
  endpoints: (build) => ({
    ...
    >({
      ...
      async onQueryStarted({ item_ids }, { dispatch, queryFulfilled }) {
        const finishFulfillment = startQueuePerformanceMeasure('rtk', 'item_summaries_by_ids RTK fulfillment', {
          requestedItemCount: item_ids.length,
        });
        try {
          const { data: summaries } = await queryFulfilled;
          finishFulfillment({ itemCount: summaries.length });
          ...
          const finishCache = startQueuePerformanceMeasure('cache', 'item_summaries_by_ids individual cache upserts', {
