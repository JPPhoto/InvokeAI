...
`widgets/layers` consumes the seam and owns nothing about the document. `layerTreeRows.ts` turns `compileDocumentNodes` plus per-project panel state (expansion, a name filter) into `LayerTreeRow`s that keep their identity while their node, place, effective state, sibling position and expansion are unchanged; `layerPanelRows.ts` flattens the four stacks into one list with a header per non-empty stack. `LayersTree.tsx` virtualizes that list (fixed row heights per density, the focused row always mounted), holds the single drag context, the single menu/properties host (`LayerSurfaceHost`, addressed by node id), the roving tab stop and the WAI-ARIA tree keyboard model (arrows, Home/End, Left to the parent, Right into a group, Alt+arrows to reorder and reparent, F2 to rename), and every store subscription and commit; a row receives one view model and one stable command handle. Drop targets are projected in linear time (`layerTreeDrop.ts`) and asked of `model.refusalFor` before the drop, so a locked, cyclic or too-deep landing shows its refusal inline instead of failing after the fact. Above 2,000 nodes the panel degrades explicitly: thumbnails and drag reordering switch off, every command stays. `layersPanel.budget.test.ts` pins the 2,000-node facts (exact row counts, one node visit per build, row identity across selection and a local expand, linear drop projection) and `LayersTree.browser.test.tsx` pins the mounted-row window, the rows a selection commit touches, the tab stop, and the keyboard journeys in Chromium.
