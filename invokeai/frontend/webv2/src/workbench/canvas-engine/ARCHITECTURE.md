...
`widgets/canvas/context-toolbar` is the one presentation of tool and operation controls above the surface. `CanvasContextToolbar` is a 40px row that never unmounts for a tool or an operation: identity, geometry, intensity, color, modes with the More menu at its right edge, a spacer and status keep their order, and every fixed region keeps its width and position while only its content changes; the row sits in the widget's layout beneath the center chrome islands, and staging keeps its floating bottom slot. `resolveToolbarLayout` decides from the measured width alone which regions the bar shows: identity and status compact at widths derived from the widest tool primary, then modes, color, intensity and geometry move into More in that order (the active tool's primary excepted), geometry shrinks to two slots, and only then does the primary follow — the bar overflows into the menu, never clips or wraps. Each tool supplies one `ToolPresentationAdapter` (`tool-options/*`): a component per region, the region kept in the bar, the bar width its modes need, and what only More shows; a guarded operation supplies an `OperationPresentationAdapter` that takes over modes, More and status while the tool's own regions stay mounted but inert. Adapters read and write the engine's option stores and document transactions directly; there is no state mirror. `CanvasContextToolbar.browser.test.tsx` pins the geometry per tool and width, the shared X/Y boxes, zero unmounts across every tool and an operation start, the keyboard path through the bar and the More menu, and the declared modes widths against rendered content.
