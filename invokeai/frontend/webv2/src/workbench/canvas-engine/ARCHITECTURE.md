...
Tool and operation settings live off the surface, in the right rail's bottom dock. `widgets/properties` renders the running operation first (its inputs, secondary controls and status with Apply / Cancel) and then the active tool's settings as labelled rows; `widgets/transform` edits the selected layer's transform without a tool — each field commits one undoable `patch`, and while the Transform tool holds a session the same fields drive its preview and Apply / Cancel land or drop it. Both read the engine's option stores and document transactions through one `ToolPresentationAdapter` per tool and one `OperationPresentationAdapter` per operation (`widgets/canvas/tool-presentation`): a component per settings region, no state mirror. Staging and generation lock the tool rows in place (`inert`) while an operation keeps its own controls live and Cancel always reachable. The right rail itself is three docks (`rightTop`, `right`, `rightBottom`), each an ordinary widget region with a tab strip; `shell/RightRail.tsx` owns the docks and `RightRail.browser.test.tsx` pins their layout, keyboard and resize behaviour.
