...
/**
 * The gallery's hand-made multi-selection, drawn larger with a white outline
 * over the base points. Empty when fewer than two items are selected — a
 * single selection is already marked by the gold target. A cluster click is
 * drawn by the cluster-selection traces instead; this one's per-point arrays
 * are only affordable at hand-picked sizes.
 */
