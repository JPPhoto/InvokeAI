...
describe('ref2va accelerator auto-pick', () => {
  ...
  it('requires the ref2v-token repack for the ref2va variant and excludes it otherwise', () => {
    expect(findMiniMaxH3TurboLora([REF2V_TURBO, FL2VA_TURBO], 'ref2va')).toMatchObject({ key: 'ref2v-turbo' });
    expect(findMiniMaxH3TurboLora([REF2V_TURBO, FL2VA_TURBO], 'fl2va')).toMatchObject({ key: 'turbo' });
    expect(findMiniMaxH3TurboLora([FL2VA_TURBO], 'ref2va')).toBeNull();
  });
