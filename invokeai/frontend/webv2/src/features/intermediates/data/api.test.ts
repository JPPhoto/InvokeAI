...
import { createIntermediatesPreview, getIntermediatesSummary, mapIntermediatesOperation } from './api';
...
describe('intermediates transport', () => {
  ...
  it('serializes a selection scope with snake_case targets', async () => {
    const fetchMock = fetchJson(
      {
        ...
        mode: 'safe',
        preview_id: 'p1',
        scope: { kind: 'selection', targets: [{ project_id: null, user_id: 'a' }], user_id: null },
        target_rows: 1,
      },
      201
    );
  ...
  it('maps operation progress including the unresolved counts a retry depends on', () => {
    const operation = mapIntermediatesOperation({
      completed_at: null,
      created_at: 'now',
      error: null,
      mode: 'force',
      ...
      retried_from_operation_id: 'previous',
      scope: { kind: 'everyone', targets: [], user_id: null },
      started_at: 'now',
      status: 'running',
      target_images: 3,
      target_videos: 0,
    ...
    expect(operation.progress).toMatchObject({ pendingDiskCleanup: 1, unknownSizeCount: 1, unresolvedImages: 2 });
    expect(operation.scope).toEqual({ kind: 'everyone' });
    expect(operation.retriedFromOperationId).toBe('previous');
  });
});
