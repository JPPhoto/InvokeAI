 ...
 * thing it loaded. Moves focus to the first operable element inside `element`, but only while nothing
 * holds focus: it rescues a keyboard user whose focus fell to `<body>` without pulling focus from
 * wherever someone has since moved it.
 */
export const focusIfUnclaimed = (element: HTMLElement | null): void => {
  if (!element || (document.activeElement !== null && document.activeElement !== document.body)) {
    return;
  }
