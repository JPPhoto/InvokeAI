...
class _PreparedExecRegistry:
    ...
    def register(self, exec_node_id: str, source_node_id: str, iter_context: tuple[str, ...] = ()) -> None:
        self._prepared_source_mapping[exec_node_id] = source_node_id
        self._metadata[exec_node_id] = _PreparedExecNodeMetadata(
            source_node_id=source_node_id, iter_context=iter_context
        )
    ...
    def get_iter_context(self, exec_node_id: str) -> tuple[str, ...]:
        return self.get_metadata(exec_node_id).iter_context
...
class _ExecutionMaterializer:
    ...
    def _create_execution_node_copy(
        self, node: BaseInvocation, node_id: str, iteration_index: int, iter_context: tuple[str, ...]
    ) -> BaseInvocation:
        ...
        self._state._register_prepared_exec_node(new_node.id, node_id, iter_context=iter_context)
        if node_id in self._state.executed:
            self._state.executed.discard(node_id)
            self._state.executed_history = [x for x in self._state.executed_history if x != node_id]
        return new_node
    ...
    def create_execution_node(
        ...
        for iteration_index in iteration_indexes:
            new_node = self._create_execution_node_copy(node, node_id, iteration_index, iter_context)
    ...
    def _get_or_create_reevaluated_node(
        ...
        prepared = self._state.source_prepared_mapping.get(source_node_id)
        if prepared:
            existing = next(
                (
                    prepared_id
                    for prepared_id in prepared
                    if self._state._prepared_registry().get_iter_context(prepared_id) == iter_context
                ),
                None,
            )
        ...
        parents = [source_id for source_id, _ in graph.in_edges(source_node_id)]
        iteration_mappings: list[tuple[str, str]] = []
        for parent_id in parents:
            prepared_id = self._resolve_parent_prepared_node(parent_id, graph, execution_graph, iter_context)
            if prepared_id is None:
                return None
            iteration_mappings.append((parent_id, prepared_id))
    ...
    def _defer_reevaluation(self, node_id: str, graph: nx.DiGraph, iterator_graph: nx.DiGraph) -> bool:
        ...
        for _, child_id in graph.out_edges(node_id):
            if any(
                isinstance(self._state.graph.get_node(ancestor_id), IterateInvocation)
                for ancestor_id in nx.ancestors(iterator_graph, child_id)
            ):
                return True
        return False
    ...
    def prepare(self, base_g: Optional[nx.DiGraph] = None) -> Optional[str]:
        ...
        iterator_graph = self.iterator_graph(g)
        next_node_id = next(
            (
                node_id
                ...
                and not self._defer_reevaluation(node_id, g, iterator_graph)
            ),
            None,
        )
        ...
        if isinstance(next_node, CollectInvocation):
            ...
            iterator_nodes = self.get_node_iterators(next_node_id, iterator_graph)
            if not iterator_nodes and not any(
                self._should_reevaluate_in_context(node_id, g) for node_id in next_node_parents
            ):
                for iteration_mappings in self._get_parent_iteration_mappings_without_iterators(next_node_parents):
            ...
            for prepared_iterators, iteration_mappings in valid_mappings:
                create_results = self.create_execution_node(
                    next_node_id,
                    iteration_mappings,  # type: ignore[arg-type]
                    iter_context=prepared_iterators,
                )
                if create_results is not None:
                    new_node_ids.extend(create_results)
...
class _ExecutionRuntime:
    ...
    def _build_reevaluated_iteration_path(self, source_node_id: str, iter_context: tuple[str, ...]) -> tuple[int, ...]:
        iterator_graph = self._state._iterator_graph(self._state.graph.nx_graph())
        topo = list(nx.topological_sort(iterator_graph))
        topo_index = {node_id: i for i, node_id in enumerate(topo)}
        iterator_pairs = []
        for iterator_exec_id in iter_context:
            iterator_source_id = self._state.prepared_source_mapping.get(iterator_exec_id)
            if iterator_source_id is not None:
                iterator_pairs.append((iterator_exec_id, iterator_source_id))
        iterator_pairs.sort(key=lambda pair: topo_index.get(pair[1], 0))

        path: list[int] = []
        for iterator_exec_id, _ in iterator_pairs:
            iterator_node = self._state.execution_graph.nodes.get(iterator_exec_id)
            if isinstance(iterator_node, IterateInvocation):
                path.append(iterator_node.index)
        return tuple(path)
    ...
    def get_iteration_path(self, exec_node_id: str) -> tuple[int, ...]:
        ...
        source_node = self._state.graph.get_node(source_node_id)
        iter_context = self._state._prepared_registry().get_iter_context(exec_node_id)
        if iter_context and getattr(type(source_node), "reevaluate_on_iteration", False):
            return self._cache_iteration_path(
                exec_node_id, self._build_reevaluated_iteration_path(source_node_id, iter_context)
            )
...
class GraphExecutionState(BaseModel):
    ...
    def _register_prepared_exec_node(
        self, exec_node_id: str, source_node_id: str, iter_context: tuple[str, ...] = ()
    ) -> None:
        self._prepared_registry().register(exec_node_id, source_node_id, iter_context=iter_context)
    ...
    def _create_execution_node(
        ...
        return self._materializer().create_execution_node(
            node_id, iteration_node_map, iteration_path=iteration_path, iter_context=iter_context
        )
