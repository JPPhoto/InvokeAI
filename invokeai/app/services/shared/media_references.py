...
Project documents and library workflows are opaque JSON to the server, but the assets they name
are not opaque to cleanup: an intermediate a saved document still points at (a canvas staging
candidate, a cropped reference image) must not be collected as garbage. The `media_references`
table is that index. It is written in the same transaction as the document it describes, so a
...
MediaReferenceOwnerKind = Literal["project", "workflow"]
