...
`literal collection source -> Iterate -> one ordinary body -> Collect`, plus its exact five-node extension with one
ordinary downstream consumer and exact six-node extension with two ordinary downstream consumers from
`Collect.collection`, is planned by a private graph-state planner, which owns its prepared-copy expansion, iteration
paths, downstream admission, and explicit empty-stream barrier without calling the legacy materializer. The planner
excludes fan-in, three or more downstream consumers, nested or input-driven iterators, and all mixed control flow;
those shapes retain materializer copy expansion, grouping, and empty-source handling on the legacy compatibility route.
...
identity, so legacy output mirroring is idempotent. The exact fresh four-node source/Iterate/body/Collect shape, its
exact five-node extension with one ordinary downstream consumer, and its exact six-node extension with two ordinary
downstream consumers use the private planner described above; fallback shapes still let the materializer create
...
the supported one- or two-consumer shapes or full `Collect` migration: fallback shapes still let the materializer own copy
expansion, iteration paths, collector grouping, collection-input hydration, and empty-source closure; the exact fresh
four-node source/Iterate/body/Collect shape, its five-node one-consumer extension, and its six-node two-consumer
extension use the private planner instead.
...
shape, its exact five-node extension with one ordinary downstream consumer, and
its exact six-node extension with two ordinary downstream consumers use the private planner for expansion and
empty-stream closure; workflow-call
