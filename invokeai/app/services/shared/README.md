...
- Otherwise: compute all combinations of prepared iterator ancestors. For each combination, choose the prepared parent
  ...
  prepared exec node for that exact iterator context. Then create **one** exec node for the current combination. If a
  node no longer has visible iterator ancestors because the source path crosses a collector, prepared parent iteration
  paths are still used to materialize one downstream exec node for each preserved collector path.
