...
branch, or exactly two or exactly three independent ordinary-node sibling `If`s compile opaque, frame-local activation
...
nested `If`s, fan-out, mixed, loop-containing, saved-workflow, legacy, and four-or-more sibling shapes remain on the
...
bounded three-`If` inner/middle/outer chain, or exactly two or exactly three independent sibling `If`s,
`GraphExecutionState` compiles opaque, frame-local activation-dependency records privately on each branch-local plan
node. Four-or-more nesting, four-or-more sibling `If`s, fan-out, mixed, loop-containing, and saved-workflow shapes use
