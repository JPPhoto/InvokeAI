...
- Run focused Python tests with `uv run --no-sync pytest <test-paths>`; the full CI suite is `uv run --no-sync pytest -n logical` (xdist; each file's tests stay on a single worker). Slow tests are excluded by default and no job runs them, so marking a test `slow` removes it from automated protection; hardware validation is separate.
